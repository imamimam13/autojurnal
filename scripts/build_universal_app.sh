...
TMP_APPLESCRIPT="$SCRIPT_DIR/build/launcher.applescript"

echo "=== Building Universal AutoJurnal.app (Intel x86_64 & Apple Silicon arm64) ==="

# 1. Generate AppIcon.icns
echo "1. Generating high-resolution AppIcon..."
"$ROOT_DIR/venv/bin/python" "$SCRIPT_DIR/generate_icon.py"

# 2. Write AppleScript Launcher
echo "2. Compiling AppleScript Universal Applet..."
mkdir -p "$SCRIPT_DIR/build"

cat <<'EOF' > "$TMP_APPLESCRIPT"
on run
...
# 3. Compile with osacompile into universal .app
echo "3. Compiling bundle with osacompile..."
rm -rf "$APP_DIR"
osacompile -o "$APP_DIR" "$TMP_APPLESCRIPT"

# 4. Replace AppIcon
echo "4. Setting AppIcon.icns..."
cp "$SCRIPT_DIR/build/AppIcon.icns" "$APP_DIR/Contents/Resources/applet.icns"
touch "$APP_DIR"

# 5. Clear quarantine attributes
echo "5. Clearing quarantine attributes..."
xattr -cr "$APP_DIR" || true

echo "✅ Universal AutoJurnal.app successfully built at: $APP_DIR"
